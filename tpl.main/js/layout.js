...
$(function(){
	...
	$('#contact').contactable({
		...
		callback:tpl_uri + '/mail.php'
	});
	
	//Add page's sidebar
	$('#content section.sidebar.related').first().detach().prependTo($('#sidebar')).removeClass('sidebar');
	$('#content section.sidebar.related > div > ul').detach().appendTo($('#sidebar .related > div > ul'));
	...
	if (i > 2) {
		toc_html = '<section class="toc"><h1>On this Page</h1><div><ul>' + toc_html + '</ul></div></section>';
