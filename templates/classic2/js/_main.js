...
YAHOO.util.Event.addListener(window, 'load', function(e) {
	...
	YAHOO.ll.panel.page.widget = new YAHOO.widget.Panel('pageContainer', {
		width: '<{$body_width}>px',
		height: YAHOO.util.Dom.getViewportHeight()-<{$header_height}>-50+'px',
		x: Math.round((YAHOO.util.Dom.getViewportWidth()-<{$body_width}>)/2),
		y: <{$header_height}>+50,
		underlay: 'none',
		close: false,
		draggable: false,
	...
	YAHOO.util.Event.addListener(window, 'resize', function(e) {
		...
		{
			cfg.setProperty('x', Math.round((YAHOO.util.Dom.getViewportWidth()-<{$body_width}>)/2));
			cfg.setProperty('height', YAHOO.util.Dom.getViewportHeight()-<{$header_height}>-<{$footer_height}>-5+'px');
		}
	});
